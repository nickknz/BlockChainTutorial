...
class Block:
    ...
    # 这个设计的目的是将具体的计算逻辑（静态方法 _compute_transactions_merkle_root）与实例数据（实例方法 compute_transactions_merkle_root）分离
    @staticmethod
    def _compute_transactions_merkle_root(transactions: List[Transaction]) -> str:
        # hash all the transactions together
